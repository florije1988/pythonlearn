...
class Mysql(object):
    ...
    def __get_conn():
        ...
        if Mysql.__pool is None:
            ...
            #                   db=Config.DBNAME,use_unicode=False,charset=Config.DBCHAR,cursorclass=DictCursor)
            __pool = PooledDB(creator=MySQLdb, mincached=1, maxcached=20,
                              host=config.local_config['database']['host'],
                              port=config.local_config['database']['port'],
                              user=config.local_config['database']['user'],
                              passwd=config.local_config['database']['passwd'],
                              db=config.local_config['database']['db'])
...
if __name__ == '__main__':
    mysql = Mysql()
    blogs = mysql.get_all('SELECT * FROM `user`;')
    print 'end'